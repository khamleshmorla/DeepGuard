...
function addToHistory(result: AnalysisResult, fileType: "image" | "video") {
  try {
...
export async function analyzeMedia(file: File): Promise<AnalysisResult> {
  ...
  try {
    ...
    const result: AnalysisResult = {
      ...
      detectedTechniques: [],
      authenticityMarkers: [],
      explanation: `Analysis performed by ${data.engine.primary}${data.engine.secondary ? ` with ${data.engine.secondary}` : ""
        }`,
      technicalNotes: `Primary engine: ${data.engine.primary}, Secondary: ${data.engine.secondary || "N/A"
        }`,
    };

    // Save to local history
    addToHistory(result, isVideo ? "video" : "image");
...
FORENSIC BREAKDOWN:
- Facial Analysis: ${typeof result.details.facialAnalysis === "object"
      ? result.details.facialAnalysis.score
      : result.details.facialAnalysis
    }%
- Temporal Consistency: ${typeof result.details.temporalConsistency === "object"
      ? result.details.temporalConsistency.score
      : result.details.temporalConsistency
    }%
- Artifact Detection: ${typeof result.details.artifactDetection === "object"
      ? result.details.artifactDetection.score
      : result.details.artifactDetection
    }%
- Metadata Analysis: ${typeof result.details.metadataAnalysis === "object"
